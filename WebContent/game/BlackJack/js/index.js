...
function passwdcheck() {
		...
		if (confirm("탈퇴하겠습니까?")) {
			...
			alert("취소되었습니다");
			window.location.href = 'memberdetailForm.action';
			return ACfalse;
		}
	}
function checkForm() {
...
function findidcheckIt(){
	...
	if(!document.findidform.jumin1.value){
		alert("주민등록번호를 입력하세요");
		document.findidform.jumin1.focus();
		return false;
	}
	if(!document.findidform.jumin2.value){
		alert("주민등록번호를 입력하세요");
		document.findidform.jumin2.focus();
		return false;
	}
}
function findid() {
	var url = "findidForm.action";
	window
			.open(
					url,
					"post",
...
function findpassword() {
	var url = "findpasswordForm.action";
	window
			.open(
					url,
					"post",
...
function openConfirmId(joinform) {
	...
	url = "checkidAction.action?id=" + joinform.id.value;
	open(
			url,
			"confirm",
...
function openmodifyPassword() {
	var url = "modifypasswdForm.action";
	window
			.open(
					url,
					"post",
